...
- proxy : str
    - if you do not spcify a proxy, the request will be sent from your IP
...
async def process_api_requests_from_file(
    ...
    logging_level: int,
    proxy: str='',
    path_to_log: str='my_log_file.log'
):
    """Processes API requests in parallel, throttling to stay under rate limits."""
    # constants
    seconds_to_pause_after_rate_limit_error = 15
    seconds_to_sleep_each_loop = (
        1#0.001  # 1 ms limits max throughput to 1,000 requests per second
    )
    ...
    # Добавление FileHandler для записи в файл
    file_handler = logging.FileHandler(path_to_log)
    file_handler.setLevel(logging.DEBUG)
    file_handler.setFormatter(logging.Formatter('%(asctime)s - %(name)s - %(levelname)s - %(message)s'))
    # Получение логгера и добавление обработчика файла
    ...
    async def call_api(
        ...
        try:
            async with session.post(
                url=request_url, proxy=proxy, headers=request_header, json=self.request_json
            ) as response:
        ...
        if error:
            ...
            if self.attempts_left:
                ...
                data = (
                    [self.event_id, self.request_json, [str(e) for e in self.result], self.metadata]
                    if self.metadata
                    else [self.event_id, self.request_json, [str(e) for e in self.result]]
                )
            ...
            data = (
                [self.event_id, response, self.metadata]
                if self.metadata
                else [self.event_id, response]
            )
            append_to_jsonl(data, save_filepath)
            status_tracker.num_tasks_in_progress -= 1
...
if __name__ == "__main__":
    asyncio.run(
        process_api_requests_from_file(
            requests_filepath='data_load.jsonl',
            save_filepath='data_save.jsonl',
